...
import sys
import traceback
import requests  # Para enviar el JSON a tu propio servidor
...
try:
    ...
    # ===============================
    print("⏳ Subiendo menu.json a tu servidor local...")
    ruta_archivo_local = r"c:\posred\bot\web\menu.json"
    
    # RUTA CORREGIDA: Carpeta /embajada/json en la torre
    url_servidor = "https://nube.menwapp.com/upload/embajada/json"
    
    with open(ruta_archivo_local, 'rb') as f:
        archivo_payload = {'file': ('menu.json', f, 'application/json')}
        response = requests.post(url_servidor, files=archivo_payload)
    
    if response.status_code == 200:
        print("✅ ¡Subido con éxito a tu propio servidor!")
        print("Respuesta:", response.json())
    else:
        print(f"⚠️ Alerta: Falló la subida a tu servidor. Código: {response.status_code}")

    # ===============================
    # SUBIR A SUPABASE (RESPALDO)
    # ===============================
    print("⏳ Guardando respaldo en Supabase...")
    subir_archivo("menu.json", ruta_archivo_local)
